...
public class IndexController {
    ...
    public ModelAndView main(ModelAndView mv) {
        ...
        User user = userService.selectByPrimaryKey(itdragonUtils.getSessionUser().getId());
        mv.addObject("user", user);
        List<Forum> forumList = forumService.getList(new Forum());
        mv.addObject("forumNum", forumList.size());
        List<Notice> noticeList = noticeService.getList(new Notice());
        mv.addObject("noticeNum", noticeList.size());
        String number = userService.zumNumber();
        mv.addObject("userNum", number);
        Integer integer = logService.num("登录");
        mv.addObject("logNum", integer);
        //最新文章
        Page<Forum> forumPage = forumService.selectPage(new Forum(), 0, 8);
        mv.addObject("forumList", forumPage.getRecords());
