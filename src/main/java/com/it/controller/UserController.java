...
public class UserController {
    ...
    public TableResultResponse userTables(User user, int page, int limit) {
        ...
        for (User userEntity : pageInfo.getRecords()) {
            ...
            if (userEntity.getStatus() == 1) {
                userMap.put("status", "<span class=\"badge-text badge-text-small success\">\n" +
                        "                         启用\n" +
                        "                    </span>");
            } else if (userEntity.getStatus() == 0) {
                userMap.put("status", " <span class=\"badge-text badge-text-small danger\">\n" +
                        "                      禁用\n" +
                        "                    </span>");
            }
    ...
    public ResultResponse userRegister(FAdmin user) {
        System.out.println(user);
//        User checkUser = userService.getUserByUserName(user.getUser_name());
//        if (checkUser != null) {
//            return Result.resuleError("用户名已存在");
//        }
//        boolean result = userService.insert(user);
